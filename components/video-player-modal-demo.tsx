...
export function VideoPlayerModalDemo({
  videoUrl,
  onClose,
  freelancerName = "Freelancer",
  gigTitle = "Project intro",
  requestHref = "/messages",
}: {
  videoUrl: string;
  onClose: () => void;
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-sm font-semibold uppercase tracking-[0.2em] text-white transition hover:border-cyan-300 hover:text-cyan-100"
