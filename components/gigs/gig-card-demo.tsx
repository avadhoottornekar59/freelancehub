...
import { VideoPlayerModalDemo } from "@/components/video-player-modal-demo";
import { VerifiedBadge } from "@/components/ui/verified-badge";
import { formatCurrency } from "@/lib/utils";

interface GigCardDemoProps {
  gig: {
    _id: string;
    title: string;
    description: string;
...
export function GigCard({ gig }: GigCardDemoProps) {
    ...
    <>
        ...
        <div className="relative h-52">
          ...
          {gig.videoIntroUrl ? (
            <button
              ...
              className="absolute left-1/2 top-1/2 inline-flex h-14 min-w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-red-600/90 px-4 text-sm font-semibold uppercase tracking-[0.2em] text-white shadow-lg transition hover:scale-105 hover:bg-red-500"
      ...
      {showVideoIntro && gig.videoIntroUrl ? (
        <VideoPlayerModalDemo
          ...
          gigTitle={gig.title}
          requestHref={`/messages?gigId=${gig._id}`}
        />
      ) : null}
    </>
  );
